...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if(type === 'nonce') {
        // permitted without authentication. Do not require an account since
        // anyone can request for a nonce.
        ...
        // all other types require authentication to create(Require an account)
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      // if token is undefined `brAuthnToken.get` throws a 404 error
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if(!authenticatedMethods.includes('token-client-registration')) {
        ...
        const clientId = req.cookies.cid;
        // if clientId exists, check if client has been registered. If it is
        // registered, then add `token-client-registration` to the list of
      ...
      if(authenticationMethod === 'token-client-registration') {
        ...
        const maxAge = cfg.cid.defaults.maxAge;
        res.setHeader('Set-Cookie', `cid=${clientId}; expires=${maxAge};` +
          `HttpOnly`);
