...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      // if a token does not exist `brAuthnToken.get`
      // returns fake hash parameters.
      let token;
      try {
        token = await brAuthnToken.get({accountId, email, type});
      } catch(e) {
        const key = cfg.delay.hmacKey;
        const salt = crypto.createHmac('sha256', key).update(email).digest();
        const b64Salt = _toBase64NoPad(salt);
        console.log('base: ', b64Salt);
        token = {
          hashParameters: {
            id: 'pbkdf2-sha512',
            params: {i: 100000},
      ...
      const delay = Math.random() * cfg.delay.jitter;
      await _sleep({delay});
...
function _toBase64NoPad(x) {
  const b32 = Buffer.from(x);
  const b16 = Buffer.alloc(16);
  b32.copy(b16, 0, 0, 16);
  return b16.toString('base64').split('=')[0];
}
