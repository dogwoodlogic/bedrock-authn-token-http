...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      // determine what to verify based on `req.body.type`
      const {type, email, hash, challenge} = req.body;
      let result;

      if(type === 'nonce' || type === 'password') {
        result = await brAuthnToken.verify({
          actor: null, result, email, type, hash, challenge,
      ...
      // update authenticated methods (re-fetch data to ensure another
      // request hasn't changed it)
      const {authenticationMethod} = result.token;
      data = _getSessionAuthenticationData(req);
      data.account = result.id;
      if(!data.authenticatedMethods) {
        data.authenticatedMethods = [authenticationMethod];
      } else if(!data.authenticatedMethods.includes(authenticationMethod)) {
        data.authenticatedMethods.push(authenticationMethod);
      }
      ...
      res.send({account, authenticated: true});
    });
